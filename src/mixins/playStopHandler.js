...
const metronomeHandler = {
  ...
  methods: {
    ...
    loadSound() {
      ...
      this.sounds.click.high = new Tone.Player(
        `../media/sounds/click/1.mp3`
      ).connect(this.vol);
      this.sounds.click.low = new Tone.Player(
        `../media/sounds/click/0.mp3`
      ).connect(this.vol);

      this.sounds.hiHat.high = new Tone.Player(
        `../media/sounds/hi-hat/1.mp3`
      ).connect(this.vol);
      this.sounds.hiHat.low = new Tone.Player(
        `../media/sounds/hi-hat/0.mp3`
      ).connect(this.vol);

      this.sounds.rimShot.high = new Tone.Player(
        `../media/sounds/rim-shot/1.mp3`
      ).connect(this.vol);
      this.sounds.rimShot.low = new Tone.Player(
    ...
    visualStart(clsName) {
      ...
      if (beat && beat.classList.contains("first-beat"))
